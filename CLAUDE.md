...
- 테스트 시 반드시 `docs/testing.md`를 참조한다.
...
**사용자**
- 사이드바 메뉴를 클릭했을 때 메뉴가 표시된다.
- 사이드바 메뉴에 등록된 문서를 선택했을 때 문서의 내용이 표시된다.
- 모든 사이드바 메뉴를 확장해도 오류가 나오지 않는다.

**관리자**
- 관리자 로그인을 이메일: `test@cc.com` / 비밀번호: `1234QWER`로 로그인한다.
- 문서관리에서 새 문서 작성을 선택하면 새 문서 화면이 표시된다.
- 제목을 "의사랑 테스트" + 날짜시간으로 등록한다.
- URL 경로는 임의로 입력한다.
- 본문에 내용 입력이 된다.
- 본문에 이미지 붙여넣기가 된다.
- 저장 버튼 클릭 시 오류가 나지 않는다.
- 사용자 관리 메뉴가 작동한다.
- 메뉴 관리 메뉴가 작동한다.
