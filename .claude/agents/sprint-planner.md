...
```
Grep with pattern="<search term>" path="/.claude/agent-memory/sprint-planner/" glob="*.md"
```
2. Session transcript logs (last resort — large files, slow):
   먼저 `pwd` 명령으로 현재 프로젝트 경로를 확인한 뒤, 경로의 `/`를 `-`로 치환하여 세션 로그 경로를 구성하세요.
   예: `/Users/xxx/Documents/Sources/my-project` → `~/.claude/projects/-Users-xxx-Documents-Sources-my-project/`
```
Grep with pattern="<search term>" path="<위에서 구성한 경로>" glob="*.jsonl"
```
